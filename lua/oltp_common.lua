...
local stmt_defs = {
   ...
   -- add by hhc @2018.12.7
   add_index = {
      "ALTER TABLE sbtest%u ADD KEY index_c(c), ADD KEY index_pad(pad)"
      },
}
...
-- Bind parameters for each stmt(including confirm type, alloc buffer...)
--   param[t]           table t
--   param[t][key]      table t, stmt key
--   param[t][key][p]   table t, stmt key, param p
...
-- add by hhc @2018.12.7
function prepare_add_index()
  prepare_for_each_table("add_index")
end
...
-- add by hhc @2018.12.7
function execute_add_index()
  local tnum = get_table_num()
  stmt[tnum].add_index:execute()
end
